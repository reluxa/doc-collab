...
describe("Collaborative editing", () => {
  ...
  it("merges edits in different sections without interference", () => {
    ...
    // Set the initial document via CRDT with the browser edit already applied.
    // This avoids .type() / insertContent() races in CI — the CRDT path is
    // the same one remote peers use and is inherently race-free.
    cy.task("collabSetTwoSectionDocument", {
      documentId: COLLAB_DOC_ID,
      sectionABody,
      sectionBBody: `${sectionBBody} edited-in-browser`,
    });

    cy.get(".ProseMirror", { timeout: 10_000 })
      .should("contain.text", "edited-in-browser")
      .should("contain.text", sectionBBody);
